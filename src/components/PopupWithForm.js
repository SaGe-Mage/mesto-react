function PopupWithForm({title, name, button, children, isOpen, onClose, onSubmit, onClickOverlay}) {
	return(
		<section className={`popup popup_place_${name} ${isOpen ? "popup_is-opened" : ""}`} onClick={onClickOverlay}>
			<div className="popup__container">
				...
				<form name={`${name}-popup`}
					...
					{children}
					<button type="submit" name="submit" className={`popup__button popup__button_place_${name}`}>{button}
					</button>
				</form>
			</div>
		</section>
